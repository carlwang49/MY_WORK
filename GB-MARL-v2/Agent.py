...
class Agent:
    ...
    def action(self, obs, agent_status, top_level_action, model_out=False):
        ...
        if top_level_action == 0: # charging action
            ...
            action = 0.5 * torch.sigmoid(logits)
            
        if model_out:
            return action, logits
        return action

    def target_action(self, obs, agent_status, top_level_action):
