...
class GB_MARL:
    ...
    def LCB(self, actions_critics_values, actions, obs_agent_id, current_hour_agent_id, epsilon=1e-8):
        ...
        time_delta = time_remaining / time_diff
        log_term = torch.abs(torch.log2(safe_actions))

        # max(0, SoC_diff)
        SoC_diff_filter = torch.clamp(SoC_diff, min=0)
        sqrt_term = clamp_and_check_nan(torch.sqrt(SoC_diff_filter / time_delta), epsilon, actions.device)

        # 檢查 sqrt_term 中是否有 inf 值
        if np.any(np.isinf(sqrt_term.detach().cpu().numpy())):
            # 將 inf 值替換為 0
