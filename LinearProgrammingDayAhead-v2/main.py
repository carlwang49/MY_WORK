...
testing_parking_data_path = TESTING_PARKING_DATA_PATH = f'../Dataset/Sim_Parking/ev_parking_data_v1_from_2018-07-01_to_2018-12-31_{NUM_AGENTS}.csv'
...
if __name__ == '__main__':
    ...
    # Load EV request data for testing
    day_ahead_testing_schedule_df = pd.read_csv(testing_parking_data_path)
    day_ahead_testing_schedule_df['date'] = pd.to_datetime(day_ahead_testing_schedule_df['date']).dt.date
    day_ahead_testing_schedule_df['arrival_time'] = pd.to_datetime(day_ahead_testing_schedule_df['arrival_time'])
    day_ahead_testing_schedule_df['actual_departure_time'] = pd.to_datetime(day_ahead_testing_schedule_df['actual_departure_time'])
    ...
    while current_time < end_time:
        ...
        ev_request_testing_list = day_ahead_testing_schedule_df[day_ahead_testing_schedule_df['date'] == current_time.date()].to_dict(orient='records')

        for ev_request in ev_request_list:
            
            ev_request_testing = [ev for ev in ev_request_testing_list if ev['requestID'] == ev_request['requestID']][0]
            ...
            testing_arrival_time, testing_departure_time = ev_request_testing['arrival_time'], ev_request_testing['actual_departure_time']
            num_intervals = departure_time - arrival_time
            num_intervals_testing = testing_departure_time.hour - testing_arrival_time.hour
            ...
            if res.success:
                ...
                for i in range(num_intervals_testing):
                    action, energy, cost = 'none', 0, 0
                    if res.x[i] > 0:
                        action = 'charge'
                        energy = res.x[i]
                        cost = res.x[i] * relevant_prices[i]
                    elif res.x[num_intervals_testing + i] > 0:
                        action = 'discharge'
                        energy = res.x[num_intervals_testing + i]
                        cost = -res.x[num_intervals_testing + i] * relevant_prices[i]
            ...
            charging_records.loc[len(charging_records)] = ({
                        ...
                        'charging_time': num_intervals_testing
                    })
