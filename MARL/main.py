...
from MARL_parameter import parse_args, get_env
...
if __name__ == '__main__':
    ...
    # create MADDPG agent
    marl = MARL(dim_info, top_dim_info, args.buffer_capacity, args.batch_size, 
    ...
    for episode in tqdm(range(args.episode_num)):
        ...
        # decrease epsilon
        # marl.change_top_level_agent_parameter(args.epsilon * (1 - episode / args.episode_num), args.sigma_decay)
        # marl.top_level_agent.update_target_network(0.1)
        ...
        while env.timestamp <= env.end_time:  
            ...
            if step < args.random_steps:
                ...
                action, top_level_action = marl.select_action(obs, global_observation, env.agents_status) # select action using MADDPG
            ...
            marl.add(obs, global_observation, action, top_level_action, reward, 
            ...
            if step >= args.random_steps and step % args.learn_interval == 0:  # learn every few steps
                marl.learn(args.batch_size, args.top_level_batch_size, args.gamma, env.agents_status) # learn from the replay buffer
                marl.update_target(args.tau) # update target network
    ...
    marl.save(episode_rewards)  # save model
