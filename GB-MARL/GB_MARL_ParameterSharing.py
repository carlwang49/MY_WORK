...
from Agent_ParameterSharing import Agent, MLPNetwork
...
class MADDPG:
    ...
    def __init__(self, dim_info, capacity, batch_size, actor_lr, critic_lr, res_dir):
        ...
        obs_dim, act_dim = dim_info[next(iter(dim_info))]

        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        shared_actor = MLPNetwork(obs_dim, act_dim).to(self.device)
        shared_critic = MLPNetwork(global_obs_act_dim, 1).to(self.device)
        actor_optimizer = Adam(shared_actor.parameters(), lr=actor_lr)
        critic_optimizer = Adam(shared_critic.parameters(), lr=critic_lr)
        # create Agent(actor-critic) and replay buffer for each agent
        self.agents = {}
        self.buffers = {}
        for agent_id in dim_info.keys():
            self.agents[agent_id] = Agent(shared_actor, shared_critic, actor_optimizer, critic_optimizer)
